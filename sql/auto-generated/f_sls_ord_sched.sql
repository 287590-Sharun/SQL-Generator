...
final_joined as (
  ...
  case 
    ...
    else coalesce(order_shipment.shipped_qty_primary_uom, 0)
  end as g_allocated_qty_primary_uom,
  ...
  vbep.mbdat as g_scheduled_ship_dt_yyyymmdd,
  case 
    when we.posnr is not null then we.customer_nbr
    else we_no_posnr.customer_nbr
  end as g_ship_to_customer_nbr,
  datediff(vbep.mbdat, vbep.edatu) as g_ship_to_delivery_days,
  tvsbt.vsbed as g_shipment_mode,
  coalesce(order_shipment.shipped_qty_primary_uom, 0) as g_shipped_qty_primary_uom,
...
left join order_shipment on vbep.vbeln = order_shipment.vbeln and vbep.posnr = order_shipment.posnr
left join last_shipped_dt on vbep.vbeln = last_shipped_dt.vbeln and vbep.posnr = last_shipped_dt.posnr
left join order_invoice on vbep.vbeln = order_invoice.vbeln and vbep.posnr = order_invoice.posnr
left join tcurf_dedup on trim(tcurf_dedup.yr_mth_nbr) = trim(order_schedule.fscl_yr_prd_nbr) and trim(to_curncy_cd) = trim(vbak.waerk) and trim(from_curncy_cd) = case when trim(upper(t001.waers)) = 'RMB' then 'CNY' else trim(upper(t001.waers)) end
),
final_joined_with_flags as (
    ...
    cast(null as string) as flag_is_blanket -- TODO: review mapping
from final_joined
left join mska on mska.vbeln = final_joined.g_order_nbr and mska.posnr = final_joined.g_order_line_nbr
left join bp on bp.vbeln = final_joined.g_order_nbr and bp.posnr = final_joined.g_order_line_nbr and bp.parvw = 'RE/BP'
left join bp_alt on bp_alt.vbeln = final_joined.g_order_nbr and bp_alt.parvw = 'RE/BP'
left join po on po.vbeln = final_joined.g_order_nbr and po.posnr = final_joined.g_order_line_nbr
left join po_alt on po_alt.vbeln = final_joined.g_order_nbr
left join request_date on request_date.vbeln = final_joined.g_order_nbr and request_date.posnr = final_joined.g_order_line_nbr
left join request_date_posnr0 on request_date_posnr0.vbeln = final_joined.g_order_nbr
left join t001 on t001.bukrs = final_joined.g_order_company_cd
left join t001k on t001k.bukrs = final_joined.g_order_company_cd
left join vbfa on vbfa.vbelv = final_joined.g_order_nbr and vbfa.posnv = final_joined.g_order_line_nbr
left join vbap on vbap.vbeln = final_joined.g_order_nbr and vbap.posnr = final_joined.g_order_line_nbr
left join vbep on vbep.vbeln = final_joined.g_order_nbr and vbep.posnr = final_joined.g_order_line_nbr
left join vbak on vbak.vbeln = final_joined.g_order_nbr
left join vbup on vbup.vbeln = final_joined.g_order_nbr and vbup.posnr = final_joined.g_order_line_nbr
left join tvap on tvap.pstyv = vbap.pstyv
left join tvep on tvep.ettyp = vbep.ettyp
left join ship_lines on ship_lines.vbeln = final_joined.g_order_nbr and ship_lines.posnr = final_joined.g_order_line_nbr
left join lst on lst.vbeln = final_joined.g_order_nbr and lst.posnr = final_joined.g_order_line_nbr
left join inv on inv.vbeln = final_joined.g_order_nbr and inv.posnr = final_joined.g_order_line_nbr
left join ship_to on ship_to.vbeln = final_joined.g_order_nbr and ship_to.posnr = final_joined.g_order_line_nbr and ship_to.parvw = 'WE'
left join ship_to_alt on ship_to_alt.vbeln = final_joined.g_order_nbr and ship_to_alt.parvw = 'WE'
left join tvsbt on tvsbt.vsbed = vbak.vsbed and tvsbt.spras = 'E' and tvsbt.mandt_id = '100'
),
sto_join as (
    select
    co_key,
